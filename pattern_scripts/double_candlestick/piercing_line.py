...
IMAGE_WIDTH = 1500
IMAGE_HEIGHT = 800
OUTPUT_DIR = "piercing_darkcloud_patterns"
...
def detect_patterns(df):
    ...
    print("Scanning for Piercing Line and Dark Cloud Cover patterns...")
    for i in tqdm(range(max(SHORT_TERM_PERIOD, LONG_TERM_PERIOD)+2, len(df))):
        if len([p for p in patterns if p['pattern'] == 'piercing_line']) >= MAX_BULLISH and \
           len([p for p in patterns if p['pattern'] == 'dark_cloud_cover']) >= MAX_BEARISH:
            break
        ...
        c1_long = c1_body / c1_range >= MIN_BODY_RATIO
        c2_long = c2_body / c2_range >= MIN_BODY_RATIO
        ...
        if (
            ...
            c2['open'] < c1['close'] - MIN_GAP and
            c2['close'] > c1['open'] + MIN_GAP and
            len([p for p in patterns if p['pattern'] == 'piercing_line']) < MAX_BULLISH
        ):
            patterns.append({
                'idx': i-1,
...
def generate_chart(df, pattern_info, file_index):
    ...
    box_color = 'darkgreen' if pattern_info['pattern'] == 'piercing_line' else 'darkred'
    ax.add_patch(patches.Rectangle(
    ...
    ax.axhline(
        ...
        color='green' if pattern_info['pattern'] == 'piercing_line' else 'red',
        linestyle=':',
        alpha=0.7,
        label='Support' if pattern_info['pattern'] == 'piercing_line' else 'Resistance'
    )
...
def run_analysis(data_file):
    ...
    bullish = [p for p in patterns if p['pattern'] == 'piercing_line'][:MAX_BULLISH]
    bearish = [p for p in patterns if p['pattern'] == 'dark_cloud_cover'][:MAX_BEARISH]
