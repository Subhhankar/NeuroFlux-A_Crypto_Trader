...
# === Configuration ===
OUTPUT_DIR = "separating_line_patterns"
...
LONG_TERM_PERIOD = 50
OPEN_TOLERANCE = 0.1  # Acceptable tolerance for "same level" open
...
def detect_patterns(df):
    ...
    print("Scanning for Bullish and Bearish Separating Line patterns...")
    for i in tqdm(range(max(SHORT_TERM_PERIOD, LONG_TERM_PERIOD)+2, len(df))):
        if len([p for p in patterns if p['pattern'] == 'bullish_separating']) >= MAX_BULLISH and \
           len([p for p in patterns if p['pattern'] == 'bearish_separating']) >= MAX_BEARISH:
            break
        ...
        if (
            ...
            abs(c2['open'] - c1['open']) <= OPEN_TOLERANCE or c2['open'] > c1['open'] and
            trends['short_term'] == 'up' and
            trends['long_term'] == 'up' and
            len([p for p in patterns if p['pattern'] == 'bullish_separating']) < MAX_BULLISH
        ):
            patterns.append({
                'idx': i-1,
        ...
        # === Bearish Separating Line ===
            ...
            abs(c2['open'] - c1['open']) <= OPEN_TOLERANCE and
...
def generate_chart(df, pattern_info, file_index):
    ...
    box_color = 'green' if pattern_info['pattern'] == 'bullish_separating' else 'red'
    ax.add_patch(patches.Rectangle(
    ...
    ax.axhline(
        ...
        color='green' if pattern_info['pattern'] == 'bullish_separating' else 'red',
        linestyle=':',
        alpha=0.7,
        label='Continuation Level'
    )
...
def run_analysis(data_file):
    ...
    bullish = [p for p in patterns if p['pattern'] == 'bullish_separating'][:MAX_BULLISH]
    bearish = [p for p in patterns if p['pattern'] == 'bearish_separating'][:MAX_BEARISH]
