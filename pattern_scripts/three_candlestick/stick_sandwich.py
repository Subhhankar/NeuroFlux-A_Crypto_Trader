...
# === Pattern Detection ===
def detect_stick_sandwich_patterns(df):
    patterns = []
    trend_cache = {}
    ...
    for i in tqdm(range(max(SHORT_TERM_PERIOD, LONG_TERM_PERIOD) + 3, len(df))):
        ...
        # Bullish Stick Sandwich (in downtrend)
        if (trends['short_term'] == 'down' and trends['long_term'] == 'down' and
            ...
            abs(c3['close'] - c1['close']) / c1['close'] < 0.002):
            patterns.append({
                'idx': i - 1,
        ...
        # Bearish Stick Sandwich (in uptrend)
...
def run_analysis(file):
    ...
    patterns = detect_stick_sandwich_patterns(df)

    print(f"\nDetected {len(patterns)} Stick Sandwich patterns")

    count_by_type = {'bullish_stick_sandwich': 0, 'bearish_stick_sandwich': 0}
    limit_by_type = {'bullish_stick_sandwich': 50, 'bearish_stick_sandwich': 50}
