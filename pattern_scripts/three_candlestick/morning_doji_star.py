...
def detect_morning_doji_star(df):
    ...
    for i in tqdm(range(max(SHORT_TERM_PERIOD, LONG_TERM_PERIOD) + 2, len(df))):
        ...
        if (c1['close'] < c1['open'] and
            ...
            c3['close'] > c2['open'] and c3['open'] > c2['open'] and
            c3['close'] > c1['open']):
...
def run_analysis(file):
    ...
    patterns = detect_morning_doji_star(df)

    print(f"\nDetected {len(patterns)} Morning Doji Star patterns")
    file_index = 0
