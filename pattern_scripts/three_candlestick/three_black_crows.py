...
def detect_three_black_crows(df):
    ...
    for i in tqdm(range(max(SHORT_TERM_PERIOD, LONG_TERM_PERIOD) + 3, len(df))):
        ...
        if all([
            ...
            c2['close'] < c1['close'],
            c3['close'] < c2['close'],
            c1['high'] - max(c1['open'], c1['close']) < 0.25 * (c1['high'] - c1['low']),
            c2['high'] - max(c2['open'], c2['close']) < 0.25 * (c2['high'] - c2['low']),
            c3['high'] - max(c3['open'], c3['close']) < 0.25 * (c3['high'] - c3['low']),
        ]):
            patterns.append({
                'idx': i - 1,
...
def generate_chart(df, pattern_info, file_index):
    ...
    ax.set_title("Three Black Crows Pattern", fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, f"{file_index:03d}_three_black_crows.png"), bbox_inches='tight', pad_inches=0.2)
    plt.close()
...
def run_analysis(file):
    ...
    print(f"\nDetected {len(patterns)} Three Black Crows patterns")
